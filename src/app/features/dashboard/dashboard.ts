...
@Component({
  ...
  template: `
    <div class="flex h-screen bg-gray-50">
      <aside class="w-56 shrink-0 border-r border-gray-200 bg-secundario p-4">
        <nav class="space-y-1">
          @for (item of menuVisible(); track item.ruta) {
            <a [routerLink]="item.ruta" routerLinkActive="bg-acento text-white"
               class="block rounded px-3 py-2 text-sm font-medium text-white/80 hover:bg-white/10">
              {{ item.label }}
            </a>
          }
        </nav>
      ...
      <div class="flex flex-1 flex-col">
        <header class="flex items-center gap-4 border-b border-gray-200 bg-secundario px-6 py-3">
          <span class="text-sm font-semibold text-white/70">Navbar</span>

          <div class="relative flex-1 max-w-md">
            <input type="search" placeholder="Buscar..."
                   class="w-full rounded border border-gray-300 bg-white px-3 py-1.5 text-sm focus:border-blue-500 focus:outline-none" />
          ...
          <a routerLink="/cambiar-password" class="ml-auto text-sm text-white/70 hover:text-white hover:underline">
            Cambiar contraseña
          </a>

          <app-boton variante="secundario" type="button" (click)="cerrarSesion()">Cerrar sesión</app-boton>

          <button type="button" class="flex h-9 w-9 items-center justify-center rounded-full border border-white/30 text-white/70 hover:bg-white/10"
